...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate draft-only follow-up outreach for generic lead sources."
    )
    parser.add_argument("--source", default="apollo_csv")
    parser.add_argument("--limit", type=int, default=25)
    parser.add_argument("--step", type=int, required=True, choices=(2, 3, 4))
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"followup_step: {result.followup_step}")
    print(f"eligible_leads: {result.eligible_leads}")
    print(f"skipped_missing_previous: {result.skipped_missing_previous}")
